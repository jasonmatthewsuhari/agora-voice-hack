...
@onready var item: Node2D = $World/InteractableItem
@onready var item_lid: Node2D = $World/InteractableItem/Lid
@onready var item_glow: CanvasItem = $World/InteractableItem/ClueGlow
@onready var item_badge: Node2D = $World/InteractableItem/Badge
...
func _position_prompt() -> void:
	...
	prompt.position = screen_position + Vector2(-prompt.size.x * 0.5, -118.0 + prompt.bob_offset)
